...
/*
    ...
    Configuration:
        No.of Cells     : 4
        Cell Radius     : 1500m
        No.Of users     : 9 static users + 1 moving user per cell
        User Speed      : ??
        Fading Model    :
        Path Loss Model :
...
int main( int argc, char *argv[] ) {
    ...
    double enb0X = -1.500*cellRadius, enb0Y = +0.000, enb0Z = 30.00;
    double enb1X = +0.000, enb1Y = +0.866*cellRadius, enb1Z = 30.00;
    double enb2X = +0.000, enb2Y = -0.866*cellRadius, enb2Z = 30.00;
    double enb3X = +1.500*cellRadius, enb3Y = +0.000, enb3Z = 30.00;

    // Number of users per cell
    unsigned enb0StaticUsers    = 2;    unsigned enb0MobileUsers    = 0;
    unsigned enb1StaticUsers    = 2;    unsigned enb1MobileUsers    = 0;
    unsigned enb2StaticUsers    = 2;    unsigned enb2MobileUsers    = 0;
    unsigned enb3StaticUsers    = 2;    unsigned enb3MobileUsers    = 0;
    ...
    uint32_t nodesCounter = NodeList::GetNNodes();    //Getting no. of nodes
    NodeContainer enbNodes;  enbNodes.Create(4);       //Creation of four Enbs

    NS_LOG_INFO( "eNodeB Nodes in range " << nodesCounter << "-" << NodeList::GetNNodes()-1 );    nodesCounter = NodeList::GetNNodes();
    NodeContainer ueNodeStaticCell0;    ueNodeStaticCell0.Create( enb0StaticUsers );     //static Ues for the (enb)Cell0 are 2
    ...
    */
    ...
    //-----------Gathering of all Static Ues in a single container---------------//
    ...
    */
    ...
    NodeContainer ueNodes;       // not important here as no Mobile Ues
    ...
    MobilityHelper mobilityEnb;                    // creating mobility helper for enbs
    Ptr<ListPositionAllocator> posAllocEnb  = CreateObject<ListPositionAllocator>();          // creation of position allocator object
    posAllocEnb->Add( Vector(enb0X,enb0Y,enb0Z) );                          // first cell   --0  Coordinates
    posAllocEnb->Add( Vector(enb1X,enb1Y,enb1Z) );                          // second cell  --1  Coordinates
    posAllocEnb->Add( Vector(enb2X,enb2Y,enb2Z) );                          // Third  Cell  --2  Coordinates
    posAllocEnb->Add( Vector(enb3X,enb3Y,enb3Z) );                          // Fourth Cell  --3  Coordinates
    mobilityEnb.SetMobilityModel( "ns3::ConstantPositionMobilityModel" );   // fixed position for Enbs
    mobilityEnb.SetPositionAllocator( posAllocEnb );
    mobilityEnb.Install( enbNodes );

    // Mobility for UEs                            // creating mobility helper for ues
    ...
    ueRadiusSampler->SetAttribute( "Min", DoubleValue(0.00) );               // min distance for ue is zero

    // Setup position allocator for UE associated with each cell x 4 cells
    Ptr<UniformDiscPositionAllocator> _posAllocStaticUeCell0  = CreateObject<UniformDiscPositionAllocator>();  // dist in done for ues having z = 0, rho random value as defined above
   ...
   /*
    ...
    for( unsigned ueIdx = 0; ueIdx < enb1MobileUsers; ueIdx++ ) { posAllocMobileUeCell1->Add( Vector(enb1X,enb1Y,1.50) ); }
    ...
    for( unsigned ueIdx = 0; ueIdx < enb2MobileUsers; ueIdx++ ) { posAllocMobileUeCell2->Add( Vector(enb2X,enb2Y,1.50) ); }
    ...
    for( unsigned ueIdx = 0; ueIdx < enb3MobileUsers; ueIdx++ ) { posAllocMobileUeCell3->Add( Vector(enb3X,enb3Y,1.50) ); }
...
*/
    ...
    Config::SetDefault( "ns3::LteUePhy::TxPower", DoubleValue(24) );         // Transmission power in dBm for the Ue
    Config::SetDefault( "ns3::LteUePhy::NoiseFigure", DoubleValue(6) );     // Default 5
    Config::SetDefault( "ns3::LteEnbPhy::TxPower", DoubleValue(40) );        // Transmission power in dBm for the Enb
    ...
    lteHelper->SetPathlossModelAttribute("Environment", StringValue("Urban"));     // in cities
    Config::SetDefault ("ns3::RadioBearerStatsCalculator::EpochDuration", TimeValue (Seconds(1.00)));


    lteHelper->SetFadingModel("ns3::TraceFadingLossModel");
    lteHelper->SetFadingModelAttribute ("TraceFilename", StringValue ("workspace/bake/source/ns-3.26/scratch/Lte1CellTestbed/fading-traces/fading_trace_EVA_60kmph.fad"));
    ...
    isHlpr.Install( ueNodes );                       // container of all nodes
    ...
    ipInfUe  = epcHelper->AssignUeIpv4Address( netDevCell0Ues );
