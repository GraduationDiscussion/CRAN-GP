...
LteRlcSpecificLteMacSapUser::NotifyTxOpportunity (uint32_t bytes, uint8_t layer, uint8_t harqId)
{
  ...
  //virtual void  3yza a3raf heya implemented fen ?!!!!!
}
...
{
  m_rlc->DoReceivePdu (p); // i guess enha UL 34an fel macSap user lma yegi pdu mn elPHY by3rfo
}
...
{
  NS_LOG_FUNCTION (this << p); //kda kda m4 han7tgha fel SM
}
...
LteRlcSm::DoReceivePdu (Ptr<Packet> p)
{
  ...
  RlcTag rlcTag; // Tag to calculate the per-PDU delay from eNb RLC to UE RLC
  Time delay;
...
LteRlcSm::DoNotifyTxOpportunity (uint32_t bytes, uint8_t layer, uint8_t harqId)
{
  ...
  params.rnti = m_rnti; //elrnti m4 bt pass lel fn met3rfa fel class dahm3nah eno bynadi elclass le kol rnti!!
  params.lcid = m_lcid;
  params.layer = layer;
